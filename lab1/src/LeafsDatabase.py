...
class LeafsDatabase:
    ...
    @staticmethod
    def get_converted_leafs(leafs_data_frame) -> List[Leaf]:
        return ConvertDataFrameToLeafs.convert(leafs_data_frame)

    def create_points_from_leafs(self, leafs: List[Leaf]) -> List[Point]:
        points: List[Point] = []
        for leaf in leafs:
            points.append(leaf.make_point_from_traits(self.trait_x_id, self.trait_y_id))
        return points
    ...
    def create_train_and_test_leafs(self):
        ...
        self.train_leafs = self.get_converted_leafs(train_leafs_metadata)
        self.test_leafs = self.get_converted_leafs(test_leafs_metadata)
        ...
        self.train_leafs_a = self.get_converted_leafs(train_leafs_a_metadata)
        self.train_leafs_b = self.get_converted_leafs(train_leafs_b_metadata)
